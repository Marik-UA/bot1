...
usd_to_short = round(howmuchusd*(1/len(echanges)),4)
howmuchusd = round(howmuchusd*(2/len(echanges)),4)
...
while ordersFilled != len(echanges):
    ...
    for n in echanges_str:
        ...
        if float(bal) < float((howmuchusd)/len(echanges)):
            printandtelegram(f'{Style.DIM}[{time.strftime("%H:%M:%S", time.gmtime(time.time()))}]{Style.RESET_ALL} Not enough balance on {n}. Need {round(float((howmuchusd)/len(echanges))-float(bal),3)} {endPair} more. Current balance on {n}: {round(bal,3)} {endPair}')
            s=1
    if s==1:
        sys.exit(1)
    ...
    for n in echanges:
        ...
        printandtelegram(f'{Style.DIM}[{time.strftime("%H:%M:%S", time.gmtime(time.time()))}]{Style.RESET_ALL} Buy limit order of {round(total_crypto/len(echanges),3)} {currentPair.split("/")[0]} at {average_first_buy_price} sent to {echanges_str[i]}.')
        i+=1
    ...
    while (zz<=first_orders_fill_timeout*60*0.5 and ordersFilled!=len(echanges)):
        for exc in echanges_str:
            ...
            if order == [] and already_filled.count(exc) == 0:
                printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} {exc} order filled.")
                ordersFilled+=1
                already_filled.append(exc)
        time.sleep(1.8)
        zz+=1
    if zz>= first_orders_fill_timeout*60*0.5:
        print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} One or more order(s) not filled in approximately {first_orders_fill_timeout} minutes. Cancelling the order(s) and selling the filled amounts.")
        emergency_convert_list(currentPair,already_filled)
        for exch in echanges_str:
            if already_filled.count(exch) == 0:
                try:
                    print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} {exc} not compatible with cancelAllOrders. Cancelling manually.")
                    open_orders = ex[exch].fetchOpenOrders(currentPair)
                    ex[exch].cancelOrder(open_orders[len(open_orders)-1]['id'],currentPair)
                    print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} {exc} order successfully cancelled.")
                except Exception as e:
                    print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Error while cancelling the not-filled orders. Error: {e}")
        already_filled=[]

balance_usdt_kucoinfutures = ex['kucoinfutures'].fetch_balance()
if balance_usdt_kucoinfutures['total'][endPair] < usd_to_short:
    ex['kucoin'].transfer(endPair,amount=round(usd_to_short-balance_usdt_kucoinfutures['total'][endPair],3),fromAccount='spot',toAccount='future')
    printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} {round(usd_to_short,3)} {endPair} successfully transferred from kucoin spot (trading) to kucoin futures.")
ex['kucoinfutures'].createMarketSellOrder(currentPair+f":{endPair}",round(usd_to_short/average_first_buy_price,3),{'leverage':1,'marginType':'isolated'})
printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Delta-neutral short for {howmuchusd*(1/3)*average_first_buy_price} {currentPair.split('/')[0]} successfully sent to kucoin futures. Waiting 120 seconds for fill...")
delta_neutral_order_fill_timeout = time.time() + 5000
delta_neutral_order = ex['kucoinfutures'].fetchOpenOrders(symbol=currentPair+f":{endPair}")
while delta_neutral_order != []:
    time.sleep(1)
    delta_neutral_order = ex['kucoinfutures'].fetchOpenOrders(symbol=currentPair[:-5]+f":{endPair}")
    if time.time() > delta_neutral_order_fill_timeout:
            ex['kucoinfutures'].cancelOrder(id=delta_neutral_order[0]['info']['id'])
            printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Delta-neutral short order not filled after 5000 seconds. Exiting.")
            sys.exit(1)
    ...
    while time.time() <= timeout:
        try:
            ...
            if max_bid_ex != min_ask_ex and change_usd > float(criteria_usd) and prec_ask_price != min_ask_price and prec_bid_price != max_bid_price:
                ...
                market_sell_order = ex[max_bid_ex].createMarketSellOrder(currentPair,crypto_per_transaction)
                if market_sell_order['price']==None:
                    new_order = ex[max_bid_ex].fetchClosedOrders(currentPair)
                    market_sell_order['price'] = new_order[len(new_order)-1]['price']
                actual_max_bid_price = market_sell_order['price']
                printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Sell market order filled on {max_bid_ex} for {crypto_per_transaction} {currentPair.split('/')[0]} at {market_sell_order['price']}.")

                market_buy_order = ex[min_ask_ex].createMarketBuyOrder(currentPair,crypto_per_transaction)
                if market_buy_order['price']==None:
                    new_order = ex[min_ask_ex].fetchClosedOrders(currentPair)
                    market_buy_order['price'] = new_order[len(new_order)-1]['price']
                actual_min_ask_price = market_buy_order['price']
                printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Buy market order filled on {max_bid_ex} for {crypto_per_transaction} {currentPair.split('/')[0]} at {market_buy_order['price']}.")
                ...
                if change_usd != actual_change_usd:
                    printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Filled price is different than excepted price (most of the time due to ping-delay). Profit excepted: {(Fore.RED if change_usd < 0 else Fore.GREEN) if change_usd!=0 else Style.RESET_ALL}{change_usd} {endPair}{Style.RESET_ALL} | Actual profit: {(Fore.RED if actual_change_usd < 0 else Fore.GREEN) if actual_change_usd!=0 else Style.RESET_ALL}{actual_change_usd} {endPair}")

                append_list_file('all_opportunities_profits.txt',actual_change_usd)
...
printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Selling all {sys.argv[1][:len(sys.argv[1])-5]} for {endPair} on {echanges_str}.")

for exc in crypto:
    order = ex[exc].createMarketSellOrder(currentPair,crypto[exc])
    balances = ex[exc].fetchBalance()
    usd[exc]=balances[endPair]['total']
    crypto[exc]=0
...
ex['kucoinfutures'].createMarketBuyOrder(currentPair+f":{endPair}",round(usd_to_short/average_first_buy_price,3),{'leverage':1})
printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Successfully stopped the delta-neutral future order on kucoin.")
...
printandtelegram(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Session with {currentPair} finished.\n{Style.DIM}{get_time()}{Style.RESET_ALL} Total account change since start: {total_session_profit_usd} USDT")
